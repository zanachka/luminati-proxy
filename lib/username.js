...
const {USERNAME_PREFS} = require('./consts.js');
const qw = string.qw;
...
const fields = qw`customer zone country state city zip session asn dns
...
const parse_meta = username=>parse_opt(username?.replace('lpm_meta-', ''));
...
const parse_opt = username=>{
    ...
    {
        let key = parts.shift();
        if (USERNAME_PREFS.includes(key))
            continue;
        if (abbr[key])
            key = abbr[key];
...
module.exports = {parse_opt, parse_meta, parse, calculate_username};
