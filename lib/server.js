...
const {find_iface, ensure_socket_close, req_util, Timeouts, REDIRECT_PARTS,
    ensure_socket_emit_close, is_redirect_status, req_is_ssl, Wl_ips} = lutil;
...
E.prototype.update_config = function(opt){
    ...
    if (opt.zagent && opt.logs_settings)
        this.remote_logger = this.logger.create_remote(opt);
    ...
    this.wl_ips = new Wl_ips(opt.whitelist_ips);
    if (opt.ext_proxies)
        opt.session = true;
    opt.use_flex_tls = opt.zagent && opt.tls_lib=='flex_tls';
    opt.flex_tls_reject_unauthorized = opt.zagent && opt.use_flex_tls
    ...
    this.opt.cids = [this.opt.customer_id, this.opt.account_id,
        this.opt.customer].filter(Boolean);
};
        ...
        (lb_transform_stream||socket).once('data', data=>{
            ...
            else if (protocol_byte==5)
            {
                this.worker.socks_server.connect(socket, {
                    ...
                    is_whitelisted_ip: ip=>this.wl_ips.contains(ip),
                });
            }
            else
                socket.end();
    ...
    status_message, start_time, req_chain)
{
    ...
    const timing = (req_chain||[]).map(t=>({
        ...
        ...t.end&&{receive: t.end-(t.first_byte||t.connect||start_time)},
        ...t.redir&&{redir: 1},
        end: (t.end||t.redir)-start_time,
    }));
    let str = `${remote_address}${lb_ip ? '('+lb_ip+')' : ''} ${method} `
        ...
        +`${timing.length ? stringify(timing) : ''}`;
    if (this.logger.level=='debug')
        str+=` ${stringify(headers)}`;
...
E.prototype.usage = function(response){
    ...
    const data = {
        ...
        proxy_peer: headers['x-luminati-ip']||headers['x-brd-ip'],
...
const auth2h = (fields=[], req={}, auth)=>{
    if (!auth || Object.keys(auth).length===0 || !req.headers)
        return;
    for (let p of fields)
    {
        let x_key = 'x-lpm-'+p;
        if (!req.headers[x_key] && auth[p])
            req.headers[x_key] = auth[p];
    }
};

const brd_auth2h = auth2h.bind(null, ['route']);
const user_auth2h = auth2h.bind(null, ['session', 'country', 'state',
    'city', 'asn', 'zip']);
...
E.prototype.is_whitelisted = function(req){
    ...
    if (auth_header)
    {
        ...
        if (user.startsWith('lpm_meta-'))
        {
            user_auth2h(req, username.parse_meta(user));
            delete req.headers['proxy-authorization'];
        }
                ...
                lpm_user_opt.lpm_user==this.opt.user))
        {
            delete req.headers['proxy-authorization'];
            if (lpm_user_opt)
                user_auth2h(req, lpm_user_opt);
            req.lpm_auth_type = 'lpm_user';
            return true;
        }
        if (consts.USERNAME_PREFS.some(p=>user.startsWith(p+'-')))
        {
            const pauth = username.parse(auth_header);
            brd_auth2h(req, pauth);
            const {customer, zone, password} = pauth;
            if (customer && zone && password)
            {
                const zones = this.opt.zones
                    || {[this.opt.zone]: this.opt.password};
                const is_cust = this.opt.cids?.includes(customer);
                const is_zone = zone in zones;
                // allow zone auth on dropin port on premise
                const in_wl = !this.opt.zagent && !this.opt.proxy_type
                    || this.opt.zone_auth_type_whitelist?.includes(customer);
                if (!is_cust || !is_zone || !in_wl)
                    return false;
                req.lpm_auth_type = 'zone';
                if (!this.opt.zagent)
                    return true;
            }
            if (pauth.auth=='token' && password)
            {
                const success = password==this.opt.token_auth
                    || password==lpm_token;
    ...
    const ip_whitelisted = this.wl_ips.contains(this.get_req_remote_ip(req));
    if (ip_whitelisted)
        req.lpm_auth_type = req.lpm_auth_type || 'ip';
...
E.prototype.get_req_cred = function(req, force_make=false){
    if (!req.ctx.cred || force_make)
        req.ctx.make_req_cred(this);
    return req.ctx.cred;
};

E.prototype.get_proxy_port = function(){
    const use_pp = this.opt.new_proxy_port===true &&
...
E.prototype.init_proxy_req = function(req, res){
    req.ctx.init_stats();
    req.ctx.host = this.session_mgr.get_session_host(req.ctx.session);
    if (this.router.is_bypass_proxy(req))
        return;
    req.ctx.proxy_port = req.ctx.session && req.ctx.session.proxy_port
        || this.get_proxy_port();
    res.cred = this.get_req_cred(req, true).username;
    res.port = req.ctx.port;
    res.lpm_auth_type = req.lpm_auth_type;
    req.ctx.make_response_proxy(this);
    req.ctx.make_connect_headers(this);
    if (req.ctx.session && req.ctx.session.ext_proxy)
        return;
    ...
    if (this.opt.zagent)
    {
        ...
        if (this.opt.user)
            req.ctx.connect_headers['x-lpm-user'] = this.opt.user;
    }
    req.ctx.connect_headers['x-hola-agent'] = agent;
};
...
E.prototype.route_port = function(req, res, head){
    let {h_route: retry_port} = req.ctx||{};
    if (req.retry || !retry_port)
        return;
...
function*lpm_request(_this, req, res, head, post, opt){
    ...
    try {
        ...
        let resp = yield _this.route_port(req, res, head);
        if (!resp)
...
const make_redirect_headers = (req, prev, new_parts)=>{
    req.headers = Object.assign({}, req.headers, {referer: prev});
    if (!req_is_ssl(req))
        return;
    req.headers.host = new_parts.hostname;
    req.url = new_parts.pathname+new_parts.search;
};
...
E.prototype.update_req_redirect = function(req, location){
    ...
    make_redirect_headers(req, prev_url, new_url);
    if (!req_is_ssl(req))
...
E.prototype.perr = function(id, info){
    ...
    this.logger.info('sending perr: %s', stringify({id, ...info}));
    lutil.perr(id, info);
};

const ignore_perr_lookup = zutil.bool_lookup(['conn_unknown',
    'flex_tls_certificate_verify_failed', 'conn_flex_tls_unknown',
    'conn_flex_tls_reuse_destroyed_socket', 'conn_flex_tls_bad_decrypt',
    'conn_flex_tls_sslv3_alert_close_notify']);
...
E.prototype.log_fn = function(e, ctx, source){
    ...
    else if (msg == 'Destination server certificate verify failed')
        perr_id = 'flex_tls_certificate_verify_failed';
    if (ignore_perr_lookup[perr_id])
        return;
    this.perr(perr_id, {
        error: zerr.e2s(e),
        ctx: source,
...
function*_request_new_socket(_this, task, req, res, head){
    ...
    const on_flex_tls_err = _this.log_throw_fn(this, ctx,
    ...
    const conn = yield _this.requester.request_socket(task, ctx, {
        ...
        flex_tls_reject_unauthorized: _this.opt.flex_tls_reject_unauthorized,
        on_flex_tls_err,
    });
    const socket_name = ctx.get_socket_name();
